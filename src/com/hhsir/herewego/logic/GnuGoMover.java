...
import com.hhsir.herewego.ai.gnugo.IGnuGoService;
...
public class GnuGoMover implements Runnable {
        ...
        if (playing_black || playing_white) {
            conn = new ServiceConnection() {
                ...
                public void onServiceConnected(ComponentName name, IBinder service) {
                    ...
                    try {
                        Log.i(GnuGoMover.class,"Service bound " + gnu_service.processGTP("test"));
                ...
                public void onServiceDisconnected(ComponentName name) {
                    Log.i(GnuGoMover.class,"Service unbound ");
                }
            };
    ...
    public String coordinates2gtpstr(Cell cell) {
        if (game == null) {
            Log.w(GnuGoMover.class,"coordinates2gtpstr called with game==null");
            return "";
        }
        int x=cell.x;
        if (cell.x >= 8)
    ...
    public void processWhiteMove(Cell cell) {
        try {
            ...
            Log.w(GnuGoMover.class,"problem processing white move to " + coordinates2gtpstr(cell));
        }
    }
    ...
    public void processBlackMove(Cell cell) {
        try {
            ...
            Log.w(GnuGoMover.class,"problem processing black move to " + coordinates2gtpstr(cell));
        }
    }
    ...
    public void stop() {
        try {
            ...
            Log.i(GnuGoMover.class,"gugoservice stopping service" + game.isFinished());
        ...
        while (!game.isFinished()) {
            ...
            if (!gnugo_size_set)
                try {
                    ...
                    Log.i(GnuGoMover.class,"setting level " + gnu_service.processGTP("level " + level));
                    gnugo_size_set = true;
                } catch (RemoteException e) {
                }
            ...
            if (isMoversMove()) {
                ...
                if (game.isBlackToMove()) {
                    try {
                        ...
                        Log.i(GnuGoMover.class,"gugoservice" + gnu_service.processGTP("showboard"));
                    } catch (RemoteException e) {
                    }
                } else {
                ...
                Log.i(GnuGoMover.class, "" + time_spent + " - " + (time_spent / 1000f));
                thinking = false;
            }
